...
public extension DiscordClient {
    ...
    #warning("terrible naming? 'addOwnMessageReaction' ")
    ...
    #warning("change to follow announcement channel")
    ...
    #warning("missing endpoint")


    #warning("change name to 'list'")
    ...
    #warning("missing endpoint possibly because of the dictionary problem with the openapi.yml?")
    ...
    func inviteResolve(
        ...
        return try await self.send(request: .init(
            to: endpoint,
            queries: [
                ("with_counts", with_counts.map { "\($0)" }),
                ("with_expiration", with_expiration.map { "\($0)" }),
                ("guild_scheduled_event_id", guild_scheduled_event_id.map { "\($0.value)" })
            ]
        ))
    }
    ...
    @inlinable
    func getUser(userId: UserSnowflake) async throws -> DiscordClientResponse<DiscordUser> {
...
#warning("check again for changing stuff like 'guildId' to 'id' etc...")
