...
class BackoffTests: XCTestCase {
    ...
    func testMinPastTime1() async throws {
        for _ in 0..<10 {
            let minTimePast = Double.random(in: 5...60)
            let backoff = Backoff(
                base: 2,
                maxExponentiation: 10,
                coefficient: 1,
                minTimePast: minTimePast
            )
            ...
            XCTAssertTolerantIsEqual(canPerformDouble, minTimePast)
        }
    }
    ...
    func testMinPastTime2() async throws {
        for _ in 0..<50 {
            for num in 1...5 {
                let minTimePast = Double.random(in: 0...5)
                ...
                XCTAssertGreaterThan(canPerformDouble, minTimePast)
    ...
    func testPreviousTry() async throws {
        for num in 1...5 {
            let minTimePast = Double.random(in: 1...5)
            let base = Double.random(in: 2...5)
            let coefficient = Double.random(in: 2...5)
            let backoff = Backoff(
                base: base,
            ...
            let timePastAfterPreviousTry = Double.random(in: 0.5..<minTimePast)
